...
POLITICS_KEYWORDS = ["후보", "공약", "출마", "선거", "의원", "당선", "유세", "국회", "총선", "지선", "대선", "국힘", "민주당","사설", "트럼프", "백악관"]
...
def summarize_with_gemini_to_html(articles: list, today_str: str) -> str:
    ...
    # 에디터 버그 방지를 위해 따옴표 3개 대신 괄호와 일반 문자열 결합을 사용합니다.
    prompt = (
    ...
    html_body = f"<h2 style='color: #2c3e50;'>📰 {today_str} AI/디지털 정책 뉴스</h2><hr style='border: 1px solid #eee; margin-bottom: 20px;'>"
    ...
    try:
        ...
        for item in summary_data:
            ...
            summary = item.get("summary", "요약 생성 중 일시적 오류 발생") 
            art = articles[idx]

            # 여기도 따옴표 3개를 제거하고 일반 문자열 결합을 사용합니다.
            html_body += (
                '<div style="margin-bottom: 25px; line-height: 1.6; font-family: \'Malgun Gothic\', sans-serif;">\n'
                f'    <a href="{art["link"]}" style="text-decoration: none; font-size: 15px; font-weight: bold; color: #03c75a;">📍 {region}</a><br>\n'
                f'    <span style="font-weight: bold; font-size: 15px; color: #333;">📌 {art["title"]}</span> <span style="font-size: 13px; color: #888;">- {art["source"]}</span><br>\n'
                f'    <span style="font-size: 14px; color: #555;">✓ {summary}</span>\n'
                '</div>\n'
            )
        ...
        for i, art in enumerate(articles):
            html_body += (
                ...
                f'    <a href="{art["link"]}" style="text-decoration: none; font-weight: bold; color: #03c75a;">📍 원문보기</a><br>\n'
                f'    <span style="font-weight: bold;">📌 {art["title"]}</span> - {art["source"]}<br>\n'
...
def main():
    ...
    subject = f"📰 {today_str} AI/디지털 정책 뉴스 리포트"
